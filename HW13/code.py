...
def webpage():
    ...
    <h1><span id="timepassed">0</span> passed</h1><br>
    ...
    <p><button class="stopwatch" name="STOPWATCH" value="STOPWATCH" type="submit">STOPWATCH</button></a></p>
    </body>
    <script>
    // LED buttons handler
    $(".button").click((el) => {{ $.post("/led", {{[el.target.name]: el.target.value}}) }});
    $(".stopwatch").click((el) => {{ $.post("/timer", {{[el.target.name]: el.target.value}}, function(val) {{ $("#timepassed").text(val) }}) }});
...
@server.route("/timer", method=HTTPMethod.POST)
def timerpress(request: HTTPRequest):
    print("timer request")
    #  get query params (doesn't handle all cases, use with caution
    query = {x[0] : x[1] for x in [x.split("=") for x in request.body.decode("utf8").split("&")]}
    #  if the led on button was pressed
    if "STOPWATCH" in query:
        #  turn on or off the onboard LED
        global times
        new_time = time.monotonic() - times
        print(str(new_time))
        times = time.monotonic()
    # Acknowledge
    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
        response.send(str(new_time))
